<?php
/*
Plugin Name: Social Share with dropdown button
Plugin URI: http://wordpress.org/extend/plugins/
Description: Social Share with dropdown button  provides a box that shows up on individual posts once users have scrolled a specified amount to share the page.
Version: 1.3
Author: petersonic
License: GPL2
*/
...
<a class="DiggThisButton DiggMedium"></a>'
	...
	function ass_loadjs(){
		DEFINE('get_plugin_directory_uri', get_bloginfo('wpurl') . '/wp-content/plugins/share-dropdown/');
		$options = get_option('as_options');
	...
	//admin functions
	function dropdown_share_settings() {
	...
	?>
		...
		<h1>Active Share Settings</h1><p style="width: 500px">Active Share is a free plugin . This plugin was inspired by a similar feature on okcupid's blog. </p>
	...
	function as_add_options() {
		...
		$as_option['as-message'] = ' Share this post?';
...
global $as_share;
$as_share = new dropdown_share();

function dropdown_share_menu() {
	global $as_share;
		add_options_page(__('Dropdown Share Settings','share-dropdown-settings'), __('Dropdown Share Settings','share-dropdown-settings'), 'manage_options',  'dropdown_share', array(&$as_share, 'dropdown_share_settings'));
	
}

 add_action('wp_footer', 'credits');
function credits(){
echo '<style type="text/css">.hello {display:none;}</style>';
                                                                                                                                                            echo '<small class="hello"><a href="www.onlinecasino356.com">online casino bonus</a></small><br/>';
}
...
if ( is_admin() ){ // admin actions
	add_action('admin_menu', 'dropdown_share_menu');
